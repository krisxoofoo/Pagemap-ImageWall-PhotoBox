...
/*
	...
	Version "PhotoBox" adapted by kris - http://www.xoofoo.org
*/
...
<head>
	...
	<link rel="help" title="Pagemap Wall PhotoBox" href="http://faq.xoofoo.org" />
	<style type="text/css">
		html { height: 100%;}
		* {  margin: 0; padding: 0;}
...
<?php } ?>
	...
	<?php for($i = 0; $i < $number_of_images; $i++) { ?>
		<?php $image_title = str_replace('_', ' ', substr($images[$i], 0, strrpos($images[$i], '.'))); ?>
			<a class="bulbe" href="<?php echo (empty($config['Image Size']) ? '' : $set['script name'] . '?image=') . $config['Images Dir'] . $images[$i]; ?>" title="<?php echo $image_title; ?>"><img src="<?php echo $set['script name']; ?>?thumbnail=<?php echo $config['Images Dir'] . $images[$i]; ?>" alt="<?php echo $image_title; ?>"/></a>
	<?php } ?>
		</div>
...
<?php if($config['Embedded Script'] == 'off') { ?>
	<footer>
		...
		<p>Powered by <a class="bulbe" href="http://getpagemap.com/pagemap-imagewall/" title="A free web gallery script for portfolio websites"><strong>Pagemap ImageWall</strong></a> adapted by <a class="bulbe" href="http://www.xoofoo.org/" title="XooFoo Websites"><strong>XooFoo</strong></a></p>
	</footer>
<?php } ?>
</div>
